...
from turtle_constants import DEG2RAD, SQRT2, L_OCC, L_FREE, L_THRESH, L_MAX, \
    COST_MAP_FREE, COST_MAP_OCC, COST_MAP_UNK, COST_MAP_INF, MAX_SCAN_RANGE
    ...
    while not rospy.is_shutdown() and not stop:
        if rospy.get_time() > t:
            ...
            for _scan_range in rbt_scan: # for each scan in one degree
                if _scan_range == 0.0: # limit scan_range from inf to MAX_SCAN_RANGE
            ...
            if et > ITERATION_PERIOD:
                print('[MAPPER] {}ms OVERSHOOT'.format(int(et*1000)))
            t += ITERATION_PERIOD
