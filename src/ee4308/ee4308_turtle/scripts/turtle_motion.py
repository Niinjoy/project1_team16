...
import sys
from turtle_constants import TWOPI, AXLE_TRACK, WHEEL_RADIUS, TRUST_ODM_V, \
    TRUST_ODM_W, TRUST_IMU_V, TRUST_IMU_W, STRAIGHT_TOLERANCE, USE_ODOM
    ...
    if USE_ODOM:
        ...
        while not rospy.is_shutdown() and not stop:
            ...
            if rospy.get_time() > t_iter: 
                ...
                if dt <= 0.:
                    print("[MOTION] dt = {} <= 0 detected. Likely due to overshoot. Skip. prev_t = {}".format(dt, prev_t))
                    continue
                ...
                filter_v = (WHEEL_RADIUS * dws / 2 / dt)*TRUST_ODM_V + \
                    (filter_v + rbt_imu_a*dt)*TRUST_IMU_V
                
                # get new filtered angular velocity
                filter_w = (WHEEL_RADIUS * dwm / AXLE_TRACK / dt)*TRUST_ODM_W + \
                    (rbt_imu_w)*TRUST_IMU_W
                ...
                if t_error > ITERATION_PERIOD:
                    print('[MOTION] {}ms OVERSHOOT'.format(int(t_error*1000)))
                # update next loop's starting time
                t_iter += ITERATION_PERIOD
            
    else:
        # use a weighted average filter between wheel encoder and noisy imu to estimate motion (x,y,o,v,w)
        ...
        while not rospy.is_shutdown() and not stop:
            ...
            if rospy.get_time() > t_iter:
                ...
                # calculate new heading from encoder
                change_imu_o = limit_angle(rbt_imu_o - prev_imu_o)
                filter_o = limit_angle(WHEEL_RADIUS * dwm / AXLE_TRACK) # actually the change
                # get new filtered heading
                filter_o = limit_angle(prev_filter_o + (filter_o*TRUST_ODM_W + change_imu_o*TRUST_IMU_W))
                    
                # get new filtered positions x and y
                if (abs(filter_w) < STRAIGHT_TOLERANCE): 
                    # filtered x and y based on low filter_w (straight path)
                    filter_x += filter_v*dt*cos(filter_o)
                    filter_y += filter_v*dt*sin(filter_o)
                else: 
                    # filtered x and y when filter_w is high (turning)
                    turn_radius = filter_v/filter_w # if filter_w is small, there will be a teleporting issue
                    filter_x += -turn_radius * sin(prev_filter_o) + turn_radius * sin(filter_o)
                    filter_y += turn_radius * cos(prev_filter_o) - turn_radius * cos(filter_o)
                
                # update the previous information for next loop
                prev_t += dt
                prev_wheel_l += dwl
                prev_wheel_r += dwr
                prev_filter_o = filter_o
                prev_imu_o += change_imu_o
